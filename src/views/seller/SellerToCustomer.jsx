...
const SellerToCustomer = () => {
 ...
 //vásárlói üzenetek valósidejű megjelenítése (server.js-ből)
 useEffect(()=>{
...
{/*Bal oldali doboz - profilkép */}
    {
     customers.map((c,i)=> <Link key={i} to={`/seller/dashboard/chat-customer/${c.fdId}`}
      ...
      items-center text-white px-2 py-2 rounded-md cursor-pointer bg-[#82b4ed]`}>
...
{/*Csevegőablak - 2. kép*/}
      ...
      items-center text-white gap-2 flex-col'>
    <span>Válaszd ki a Vásárlót a beszélgetés megkezdéséhez</span>
    </div>
}
      </div>
    </div>
  {/*Jobb oldali doboz - Bemeneti mező és gomb*/}
    <form onSubmit={send} className='flex gap-3'>
      ...
      border border-slate-700 items-center py-[5px]
      focus:border-blue-500 rounded-md outline-none
      bg-transparent text-[#d0d2d6]'
      type="text" placeholder='Írd ide az üzeneted'/>
      <button className='shadow-lg bg-[#06b6d4]
      hover:shadow-cyan-500/50 text-semibold
