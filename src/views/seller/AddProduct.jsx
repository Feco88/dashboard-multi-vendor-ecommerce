...
const AddProduct = () => {
    ...
    const { loader, successMessage, errorMessage } = useSelector(state =>
        state.product)
...
// Űrlapadatok kinyerése
    ...
    formData.append('shopName','Feca Shop')
    formData.append('category',category)
//Képek adatainak kinyerése
...
<div>
    <form onSubmit={add}>
              ...
              <div className={`absolute top-[101%] bg-[#475569] w-full
                transition-all ${cateShow ? 'scale-100' : 'scale-0 '} `}>
                 ...
                 h-[200px] overflow-x-scrool'>
                  {
                    allCategory.map((c,i) => <span className={`px-4
                     py-1 hover:bg-blue-500 hover:text-white
      ...
      <div className='flex flex-col w-full gap-1 mb-5'>
            <label htmlFor="description" className='text-[#d0d2d6]'>Termékleírás</label>
            <textarea className='px-4 py-2 focus:border-blue-500 outline-none
              bg-[#283046] border border-slate-700 rounded-md text-[#d0d2d6]'
              onChange={inputHandle} value={state.description}
              name='description' id='description' placeholder='Termékleírás hozzáadása...'
      ...
      sm:gap-4 md:gap-4 gap-3 w-full text-[#d0d2d6] mb-4'>
        {/*Termékképek megjelenítése*/}
        {
         imageShow.map((img,i) => <div className='h-[180px] relative'>
            <label htmlFor={i}>
                <img className='w-full h-full rounded-sm object-contain' src={img.url} alt="" />
            </label>
        {/*Termékképek cseréje*/}
        ...
        {/*Termékképek eltávolítása*/}
            ...
            hover:shadow-lg hover:shadow-slate-400/50 text-white
            absolute top-1 right-1 rounded-full'><IoMdCloseCircle /></span>
         </div>)
        }
        <label className='flex justify-center items-center flex-col h-[180px] cursor-pointer 
        border border-dashed hover:border-red-500 w-full text-[#d0d2d6]'
        htmlFor="image">
